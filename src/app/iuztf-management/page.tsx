...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Applications', value: stats.total_applications, icon: FileText, color: 'bg-blue-500', href: '/iuztf-management/admissions' },
    { label: 'Pending Review', value: stats.pending, icon: Clock, color: 'bg-yellow-500', href: '/iuztf-management/admissions?status=pending' },
    { label: 'Accepted', value: stats.accepted, icon: CheckCircle, color: 'bg-green-500', href: '/iuztf-management/admissions?status=accepted' },
    { label: 'Faculty Members', value: stats.faculty_count || 8, icon: Users, color: 'bg-indigo-500', href: '/iuztf-management/faculty' },
    { label: 'Blog Posts', value: stats.blog_posts || 6, icon: BookOpen, color: 'bg-purple-500', href: '/iuztf-management/blog' },
    { label: 'Gallery Images', value: stats.gallery_images || 6, icon: Image, color: 'bg-pink-500', href: '/iuztf-management/gallery' },
    { label: 'Unread Messages', value: stats.unread_messages, icon: MessageSquare, color: 'bg-red-500', href: '/iuztf-management/contact' },
    { label: 'Enrolled Students', value: stats.enrolled, icon: GraduationCap, color: 'bg-teal-500', href: '/iuztf-management/admissions?status=enrolled' },
  ];
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="hidden sm:flex gap-2">
          <Link href="/iuztf-management/admissions" className="flex items-center gap-1 bg-[#0A1628] text-white text-xs font-bold px-4 py-2 rounded-xl hover:bg-[#C9A84C] hover:text-[#0A1628] transition">
            <Plus className="w-3.5 h-3.5" /> New Admission
          </Link>
          <Link href="/iuztf-management/blog" className="flex items-center gap-1 border border-gray-200 text-gray-600 text-xs font-bold px-4 py-2 rounded-xl hover:border-[#C9A84C] transition">
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <Link href="/iuztf-management/admissions" className="mt-4 inline-flex items-center gap-1 text-xs text-[#C9A84C] font-bold hover:underline">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { icon: '📝', label: 'New Blog Post', href: '/iuztf-management/blog' },
              { icon: '🖼️', label: 'Upload Images', href: '/iuztf-management/gallery' },
              { icon: '👨‍🏫', label: 'Add Faculty', href: '/iuztf-management/faculty' },
              { icon: '📋', label: 'View Applications', href: '/iuztf-management/admissions' },
              { icon: '🏛️', label: 'Manage Institutes', href: '/iuztf-management/institutes' },
          ...
          <Link href="/iuztf-management/admissions"
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/iuztf-management/admissions" className="text-xs text-[#C9A84C] font-bold hover:underline">View All</Link>
          </div>
          {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/iuztf-management/contact" className="text-xs text-[#C9A84C] font-bold hover:underline">View All</Link>
          </div>
          {loading ? (
            <div className="p-8 text-center text-gray-400 text-sm">Loading...</div>
